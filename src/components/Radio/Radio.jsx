...
const Radio = ({ item }) => {
    ...
    const increment = () => setCount((prev) => prev < 2 ? prev + 1 : prev)
    ...
    return (
        ...
        <div className='flex justify-between' >
            <label className={s.container} onClick={checked}>{item.text}
                <p>{item.price}</p>
                <input type="radio" name='nuts' ref={test} />
            ...
            <div className='flex items-center'>
                ...
                <input type="number" max={2} min={0} defaultValue={0} disabled={!check} value={count} className={s.count} />
